...
public:
    Test(NodeManager *node, QObject *p = nullptr)
        : QObject(p)
    {
        Transaction tr(node->getAccController()->getCurrentActor().getId(), BigNumber("ddddaaaa332232"),
                       BigNumber(124));
        Transaction tr1(node->getAccController()->getCurrentActor().getId(), BigNumber("322323dddaa"),
                        BigNumber(23));
        Transaction tr2(node->getAccController()->getCurrentActor().getId(), BigNumber("234234aaaa"),
                        BigNumber(45));
        Transaction tr3(node->getAccController()->getCurrentActor().getId(), BigNumber("23aaaaaaaaaa"),
                        BigNumber(4));
